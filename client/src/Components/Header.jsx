import React from 'react'
import { Link, useNavigate } from 'react-router-dom'

const Header = () => {

  const navigate = useNavigate();

  return (
    <div>

      <header className='font-mono flex justify-around shadow-2xl'>

        <div className='py-5 w-10 h-10'>
          <h1>
            <img src="data:image/png;base64,"
              alt="" />
          </h1>
        </div>

        <ul className='py-5 flex justify-center gap-x-15'>
          <Link to='/'>
            <li className='transition-all duration-700 ease-in-out hover:text-amber-600 text-2xl cursor-pointer'>Home</li>
          </Link>

          <Link to='/about'>
            <li className='transition-all duration-700 ease-in-out hover:text-amber-600 text-2xl cursor-pointer'>About</li>
          </Link>

          <Link to='/contact'>
            <li className='transition-all duration-700 ease-in-out hover:text-amber-600 text-2xl cursor-pointer'>Contact</li>
          </Link>

          {/* onClick={ () => { navigate('/signin ')}}  */}
          <Link to='/signin'>
            <li className='transition-all duration-700 ease-in-out hover:text-amber-600 text-2xl cursor-pointer'>Sign In</li>
          </Link>

          {/*  onClick={ () => { navigate('/signup')}}  */}
          <Link to='signup'>
            <li className='transition-all duration-700 ease-in-out hover:text-amber-600 text-2xl cursor-pointer'>Sign Up</li>
          </Link>

          <Link to='/music'>
            <li className='transition-all duration-700 ease-in-out hover:text-amber-600 text-2xl cursor-pointer font-bold text-green-500'>Music</li>
          </Link>
        </ul>

      </header>

    </div>
  )
}

export default Header